...
A configuração iOS agora usa `version = 1.0.0`, `ios.bundleIdentifier = br.com.fiqueok.saudefamiliar` e `ios.supportsTablet = false`, mantendo o primeiro ciclo focado em iPhone. No `expo config`/prebuild local fora de CI e EAS, o `ios.buildNumber` usa o valor seguro `1` quando nenhuma fonte existe. Em GitHub Actions, `IOS_BUILD_NUMBER` tem prioridade, `GITHUB_RUN_NUMBER` é a fonte de fallback e ausência de fonte ou valor inválido falha claramente com a identificação da variável usada. Em builds EAS, o `eas.json` define `cli.appVersionSource = remote` e `autoIncrement = true` em `development`, `preview` e `production`, delegando a sequência ao versionamento remoto do EAS e impedindo a reutilização do fallback local. Android permanece com `package = br.com.fiqueok.saudefamiliar`, `versionCode` e permissões atuais sem alteração funcional.
