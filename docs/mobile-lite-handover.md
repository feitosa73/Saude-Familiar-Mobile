...
O workflow `Validate Mobile` roda em Pull Requests e em pushes para `main`. Ele instala as dependências com `pnpm install --frozen-lockfile --ignore-scripts`, executa o typecheck do workspace, testa a geração e serialização local do evento ICS com `test:calendar` e valida a configuração Expo. Esse teste não substitui a validação física do editor de calendário e do Share Sheet em Android e iOS. O workflow `Build installable APK` é disparado manualmente, em Pull Requests e em pushes para `main`; ele configura Node 22, pnpm 11.22, Java 17, deriva `android.versionCode` de `github.run_number`, valida e registra `Android version: 1.0.0 (N)`, executa `expo prebuild --platform android --no-install --clean`, gera `assembleRelease` e publica o APK com checksum por 14 dias.
...
## 14. Compartilhamento de Agendamentos / Calendário — Sprint 5

A Sprint 5 permite compartilhar uma Consulta ou Exame local já cadastrado como evento de calendário, sem sincronização entre usuários, sem backend, sem nuvem e sem alterar o modelo Local-Only. A ação aparece somente para registros com `status = scheduled` e data e hora civis válidas. Agendamentos `pending`, `completed` e `cancelled`, bem como registros sem data ou sem hora, não oferecem compartilhamento.

A opção `Adicionar ao meu calendário` usa o editor nativo do sistema por meio do `expo-calendar`. A permissão de calendário é solicitada somente quando o usuário escolhe essa ação, e a recusa apenas informa que o agendamento não foi adicionado, sem impedir o uso do aplicativo. O módulo não lista, lê, altera ou sincroniza eventos existentes; no Expo SDK 54, a configuração oficial do módulo declara as permissões de calendário exigidas pelo editor nativo para Android e iOS.

A opção `Compartilhar com alguém` gera um arquivo temporário `.ics` no cache privado do aplicativo e o envia ao Share Sheet nativo por meio do `expo-sharing`, com MIME `text/calendar` e UTI de calendário no iOS. O Saúde Familiar não acessa contatos, e-mail ou WhatsApp, não escolhe destinatários, não armazena histórico e não envia informações a servidor. O arquivo é removido do cache depois que o Share Sheet termina, inclusive quando ocorre falha.

O evento usa título `Consulta - {especialidade} - {nome}` ou `Exame - {nome do exame} - {nome}`. O nome do Patient é incluído para diferenciar familiares no calendário. A descrição é fixa e curta: `Agendamento compartilhado pelo Saúde Familiar.`. O local é incluído somente quando preenchido. Profissional, telefone, observações, diagnóstico, sintomas, resultados, informações clínicas e outros campos sensíveis são deliberadamente excluídos.

O `.ics` contém `BEGIN:VCALENDAR`, `VERSION:2.0`, `PRODID`, `CALSCALE`, `METHOD`, um `VEVENT` com `UID`, `DTSTAMP`, `DTSTART`, `DTEND`, `SUMMARY`, `LOCATION` quando existente e `DESCRIPTION`, seguido pelos encerramentos correspondentes. Vírgulas, ponto e vírgula, barras invertidas e quebras de linha são escapados segundo o formato iCalendar, preservando acentos e caracteres portugueses. O UID é estável e deriva do ID local do Agendamento, com o domínio `saudefamiliar.local`; compartilhar novamente o mesmo registro mantém o mesmo UID, mas não atualiza automaticamente eventos já enviados a calendários de terceiros.

A duração padrão é de uma hora, sem nova coluna ou migration. A data e a hora são interpretadas como horário civil local, reutilizando a mesma validação do planejamento de lembretes. Assim, `2026-09-30` às `07:00` permanece `30/09/2026 às 07:00` no dispositivo, e a conversão da duração também atravessa corretamente mudança de dia, mês ou ano. Os campos `DTSTART` e `DTEND` do ICS são locais e não recebem sufixo UTC.

A integração permanece offline: gerar o evento, abrir o editor nativo e preparar o Share Sheet não dependem de Internet. A efetiva entrega ao destino escolhido pode depender da conectividade e do aplicativo externo. Não foi criada migration nem persistência adicional.

Os testes automatizados cobrem Consulta e Exame, elegibilidade por status, ausência de data ou hora, datas inválidas, acentos, caracteres especiais, local, horário de 07:00, horário próximo da meia-noite, mudança de ano, duração de uma hora, UID estável, estrutura e escaping ICS e exclusão de dados sensíveis. Ainda permanecem pendentes os testes físicos em dispositivos Android e iOS, incluindo abertura do editor nativo, permissões, Share Sheet, importação do `.ics` por clientes de calendário e comportamento em telas compactas. A validação CI cobre configuração Expo, prebuild iOS sem assinatura e APK Android; ela não substitui esses testes físicos.
