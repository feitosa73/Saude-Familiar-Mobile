...
A configuração iOS agora usa `version = 1.0.0`, `ios.bundleIdentifier = br.com.fiqueok.saudefamiliar` e `ios.supportsTablet = false`, mantendo o primeiro ciclo focado em iPhone. O `ios.buildNumber` é resolvido automaticamente: `IOS_BUILD_NUMBER` tem prioridade, `GITHUB_RUN_NUMBER` é a fonte de fallback no CI e o valor local seguro é `1` somente quando nenhuma fonte existe fora do CI. No CI, ausência de fonte e valores inválidos falham claramente com a identificação da variável usada. Android permanece com `package = br.com.fiqueok.saudefamiliar`, `versionCode` e permissões atuais sem alteração funcional.

Foi adicionado `artifacts/saude-familiar-mobile/eas.json` com perfis separados `development`, `preview` e `production`. O perfil `development` usa `expo-dev-client` e distribuição interna; `preview` usa distribuição interna; `production` permanece disponível para futura build de loja. Não há publicação automática, credenciais Apple, App Store Connect, provisioning profile, certificado, dispositivo registrado ou upload para TestFlight configurado nesta Sprint.
